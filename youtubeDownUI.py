"""
Title: YoutubeDown
Descripcion: Programa para poder descargar videos de youtube que necesito en mi caso para las clases de inglés ó algún
             tutorial que me interesa ver más tarde, a una calidad 720p
             para no tener que depender de páginas de internet, que me descarguen los videos con riesgo de que me
             inserten algun virus o algo peor, y a la vez me sirva para practicar python y el uso de la libreria Pytube.
...
"""
from tkinter import Tk, Button, Entry, Label, PhotoImage, Frame, Menu, messagebox
...
def popup():
    messagebox.showinfo("Acerca de", "Programa Para descargar videos de youtube usando Pytube \n"
                                     "Autor: Tomas Rodriguez \n"
                                     "https://www.linkedin.com/in/tomas86/ ")
    ...
    def __init__(self, master, *args):
        ...
        def descarga():
            ...
            try:
                ...
                self.statusBar = Label(bd=1, fg="grey", width=55, relief=SUNKEN, anchor='center',
                                       text=" * Video de Youtube Descargado * ")
                self.statusBar.grid(column=0, row=1, columnspan=3, pady=5, padx=15)


            except:
                # print("Error en la url del video, no es posible descargar")
                self.statusBar = Label(bd=1, fg="red", width=55, relief=SUNKEN, anchor='center',
                                       text=" * Error, introduce una URL Válida * ")
                self.statusBar.grid(column=0, row=1, columnspan=3, pady=5, padx=15)
        ...
        self.boton = Button(image=self.iBoton, bg='#e2e2e2', highlightbackground='#e2e2e2', highlightthickness=2,
                            borderwidth=0, command=descarga)
...
if __name__ == "__main__":
    ...
    ventana.resizable(False, False)  # no me pueden cambiar el tamaño de la ventana
    ...
    menuBar.add_cascade(label="Inicio", menu=menuAyuda)
    menuAyuda.add_cascade(label="Acerca de", command=popup)
    menuBar.add_command(label="Cerrar", command=cerrar)
